...
export const Fretboard: React.FC<FretboardProps> = ({
  ...
  const width = 800;
  const height = 130; // Reduced height
  const margin = 15;
  const neckWidth = width - margin * 2;
  const neckHeight = height - margin * 2;
  ...
  return (
    <div className="w-full">
      ...
      >
        <rect x={margin} y={margin} width={neckWidth} height={neckHeight} fill="#111" rx="2" />
        <rect x={margin - 3} y={margin - 1} width="6" height={neckHeight + 2} fill="#e5e7eb" rx="1" />

        {Array.from({ length: numFrets + 1 }).map((_, i) => (
          <g key={`fret-${i}`}>
            <line x1={margin + i * fretSpacing + 0.5} y1={margin} x2={margin + i * fretSpacing + 0.5} y2={height - margin} stroke="#000" strokeWidth="1" opacity="0.6" />
            <line x1={margin + i * fretSpacing} y1={margin} x2={margin + i * fretSpacing} y2={height - margin} stroke="#cbd5e1" strokeWidth={i === 0 ? "0" : "1.5"} />
          </g>
        ))}

        {[3, 5, 7, 9, 12, 15, 17, 19, 21].map(fret => {
          if (fret < startFret || fret > endFret) return null;
          const x = margin + (fret - startFret - 0.5) * fretSpacing;
          if (fret === 12) {
             return (
               <g key="marker-12">
                 <circle cx={x} cy={margin + stringSpacing * 1.5} r="3" fill="#f8fafc" opacity="0.4" />
                 <circle cx={x} cy={margin + stringSpacing * 3.5} r="3" fill="#f8fafc" opacity="0.4" />
               </g>
             );
          }
        ...
        {strings.map((str, i) => (
          <g key={`string-${str}`}>
             <line x1={margin} y1={margin + i * stringSpacing + 0.5} x2={width - margin} y2={margin + i * stringSpacing + 0.5} stroke="#000" strokeWidth={0.8 + (6 - str) * 0.2} opacity="0.5" />
             <line x1={margin} y1={margin + i * stringSpacing} x2={width - margin} y2={margin + i * stringSpacing} stroke={str > 3 ? "#94a3b8" : "#f1f5f9"} strokeWidth={0.8 + (6 - str) * 0.2} opacity="0.8" />
          </g>
        ))}
        ...
        {showHeatmap && Array.from({ length: numFrets }).map((_, f) => {
          ...
          return strings.map(str => (
            <rect key={`heat-${str}-${fretNum}`} x={margin + f * fretSpacing + 1} y={margin + (str - 1) * stringSpacing - stringSpacing / 2 + 1} width={fretSpacing - 2} height={stringSpacing - 2} fill={getMasteryColor(str, fretNum)} fillOpacity="0.3" rx="1" />
          ));
        })}
